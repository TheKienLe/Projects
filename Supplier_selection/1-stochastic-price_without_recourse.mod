...
subject to 
{
  ...
  purchasing_cost == sum (s in S)( p[s] * (sum (k in K, i in I, r in R) (1 - delta[i][r] )*(f[i][k]+fdelta[i][k][s]) *(z[i][k][r] + z_plus[i][k][r][s] - z_minus[i][k][r][s])));
  total_cost == activation_cost + purchasing_cost;
